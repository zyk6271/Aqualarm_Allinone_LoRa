...
static uint8_t external_valve_warning = 0;
...
rt_timer_t valve_delay_check_timer = RT_NULL;
rt_timer_t valve_internal_check_timer = RT_NULL;
...
rt_err_t valve_open(void)
{
    ...
    rt_timer_stop(valve_delay_check_timer);
...
void valve_check(void)
{
    ...
    {
        rt_pin_write(MOTO_CONTROL_PIN,PIN_HIGH);
        rt_timer_start(valve_delay_check_timer);
    }
}

void valve_internal_check_timer_callback(void *parameter)
{
    rt_pin_write(MOTO_CONTROL_PIN,PIN_HIGH);
    if(rt_pin_read(MOTO_OPEN_STATUS_PIN))
    {
        ...
        LOG_D("valve_internal_check_timer_callback success\r\n");
    }
    else
    {
        valve_valid = 0;
        ...
        LOG_E("valve_internal_check_timer_callback failed\r\n");
    }
}
...
void valve_delay_check_timer_callback(void *parameter)
{
    uint8_t open_status = rt_pin_read(MOTO_OPEN_STATUS_PIN);
    if(open_status)
    {
        warning_enable(InternalValveFailEvent);
    }
    ...
    {
        rt_timer_start(valve_internal_check_timer);
        rt_timer_stop(valve_open_timer);
        rt_timer_stop(valve_close_timer);
        rt_pin_write(MOTO_CONTROL_PIN,PIN_LOW);
        pd_valve_check();
...
void valve_init(void)
{
    ...
    valve_internal_check_timer = rt_timer_create("valve_internal_check", valve_internal_check_timer_callback, RT_NULL, 5000, RT_TIMER_FLAG_ONE_SHOT|RT_TIMER_FLAG_SOFT_TIMER);
    valve_external_check_timer = rt_timer_create("valve_external_check", valve_external_check_timer_callback, RT_NULL, 15000, RT_TIMER_FLAG_ONE_SHOT|RT_TIMER_FLAG_SOFT_TIMER);
    ...
    valve_delay_check_timer = rt_timer_create("valve_delay_check", valve_delay_check_timer_callback, RT_NULL, 10*1000, RT_TIMER_FLAG_ONE_SHOT|RT_TIMER_FLAG_SOFT_TIMER);
    rt_timer_start(valve_detect_timer);
